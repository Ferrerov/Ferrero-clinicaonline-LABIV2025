import { Component, ElementRef, inject, ViewChild } from '@angular/core';
import { CommonModule } from '@angular/common';
import { SupabaseDbService } from '../../services/supabase.service';
import { TurnoInterface } from '../../interfaces/turno.interface';
import { UsuariosEspecialidadInterface } from '../../interfaces/usuarios-especialidad.interface';
import { EspecialidadInterface } from '../../interfaces/especialidad.interface';
import { BaseChartDirective } from 'ng2-charts';
import { ChartData, ChartType } from 'chart.js';
import { InformeInterface } from '../../interfaces/informe.interface';
import { ExcelService } from '../../services/excel.service';
import html2pdf from 'html2pdf.js';
import { logo64 } from '../../../assets/logo64';
import html2canvas from 'html2canvas';

@Component({
  selector: 'app-informe-turnos-especialidad',
  imports: [CommonModule, BaseChartDirective],
  templateUrl: './informe-turnos-especialidad.component.html',
  styleUrl: './informe-turnos-especialidad.component.scss',
  standalone: true,
})
export class InformeTurnosEspecialidadComponent implements InformeInterface {
  @ViewChild(BaseChartDirective) chart!: BaseChartDirective;
  @ViewChild('grafico', { static: false }) grafico!: ElementRef;

  supabase = inject(SupabaseDbService);
  excelService = inject(ExcelService);

  chartData: ChartData<'bar'> = {
    labels: [],
    datasets: [
      { label: 'Cantidad de turnos', data: [], backgroundColor: '#CBA368' },
    ],
  };
  chartType: ChartType = 'bar';

  async ngOnInit() {
    const turnos = await this.supabase.buscarTodos<TurnoInterface>('turnos');
    const usuariosEsp =
      await this.supabase.buscarTodos<UsuariosEspecialidadInterface>(
        'usuarios_especialidad'
      );
    const especialidades =
      await this.supabase.buscarTodos<EspecialidadInterface>('especialidad');

    const contador: Record<string, number> = {};
    for (const turno of turnos) {
      const match = usuariosEsp.find(
        (u) => u.id === turno.usuario_especialidad_id
      );
      const especialidad =
        especialidades.find((e) => e.id === match?.especialidad_id)?.nombre ??
        'Desconocida';
      contador[especialidad] = (contador[especialidad] || 0) + 1;
    }

    this.chartData.labels = Object.keys(contador);
    this.chartData.datasets[0].data = Object.values(contador);
    setTimeout(() => this.chart?.update());
  }

  descargarExcel() {
    const exportData = (this.chartData.labels as string[]).map(
      (label, idx) => ({
        Especialidad: label,
        Turnos: this.chartData.datasets[0].data[idx],
      })
    );
    this.excelService.exportAsExcelFile(exportData, 'turnos_por_especialidad');
  }

  async descargarPDF() {
    const fecha = new Date().toLocaleDateString();
    const labels = this.chartData.labels as string[];
    const data = this.chartData.datasets[0].data as number[];

    const canvas = await html2canvas(this.grafico.nativeElement, { scale: 2 });
    const img = canvas.toDataURL('image/png');

    const contenedor = document.createElement('div');
    contenedor.style.backgroundColor = '#061630';
    contenedor.style.padding = '20px';
    contenedor.style.color = '#EAF4F8';
    contenedor.style.minHeight = '1122px';
    contenedor.innerHTML = `
      <div style="display: flex; align-items: center;">
        <img src="${logo64}" style="height: 50px; margin-right: 15px;">
        <h1 style="color: #CBA368;">Clínica Online</h1>
      </div>
      <div style="margin-bottom: 10px;">Fecha de emisión: ${fecha}</div>
      <h3 style="color: #CBA368;">Cantidad de turnos por especialidad</h3>
      <ul>${labels.map((l, i) => `<li>${l}: ${data[i]}</li>`).join('')}</ul>
      <img src="${img}" style="width: 100%; margin-top: 20px;">
    `;

    html2pdf()
      .set({
        margin: 0,
        filename: `turnos_por_especialidad_${fecha}.pdf`,
        image: { type: 'jpeg', quality: 0.98 },
        html2canvas: { scale: 2, backgroundColor: '#061630' },
        jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' },
      })
      .from(contenedor)
      .save();
  }
}
